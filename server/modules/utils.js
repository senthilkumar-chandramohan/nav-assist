...
const getDataSetFilenameFragment = (dataSetName) => {
    ...
    switch (dataSetName) {
        case "AtoZ": fileNameFragment = "_A_Z_dataset"; break;
        case "atoz": fileNameFragment = "a_z_dataset"; break;
        case '0to9SplChars': fileNameFragment = "0_9_spl_chars_dataset"; break;
...
const writePixelDataToCSV = (dataSetName) => {
    const header = [
        "label",
        "pixel1",
        "pixel2",
    ...
    fs.writeFile(path.resolve(path.join(__dirname, `../data/${getDataSetFilenameFragment(dataSetName)}_train.csv`)), trainingCSV, "utf8", function (err) {
        if (err) {
    ...
    fs.writeFile(path.resolve(path.join(__dirname, `../data/${getDataSetFilenameFragment(dataSetName)}_test.csv`)), testingCSV, "utf8", function (err) {
        if (err) {
          console.log("Some error occured - file either not saved or corrupted file saved.");
        } else {
...
const extractPixelDataFromImages = (dataSetName) => {
    ...
    if (dataSetName === "AtoZ") {
    ...
    } else if (dataSetName === "atoz") {
        for (let i=0; i<26; i++) {
            var alphabet = String.fromCharCode(i + 97); // char code of 'a' is 97
            labelFolderMap.push({
                label: alphabet,
                folder: alphabet,
            });
        }
    } else if (dataSetName === '0to9SplChars') {
        for (let i=0; i<=9; i++) {
            labelFolderMap.push({
                label: i.toString(),
                folder: i.toString(),
        ...
        labelFolderMap.push({ label: ".", folder: "period" });
    }
